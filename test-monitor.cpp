#include <gtest/gtest.h>
#include "./monitor.h"

// Temperature tests
TEST(MonitorTest, TemperatureHighNotOk) {
  EXPECT_FALSE(vitalsOk(103.0f, 75.0f, 95.0f));
}

TEST(MonitorTest, TemperatureLowNotOk) {
  EXPECT_FALSE(vitalsOk(94.0f, 75.0f, 95.0f));
}

TEST(MonitorTest, TemperatureNormalOk) {
  EXPECT_TRUE(vitalsOk(98.6f, 75.0f, 95.0f));
}

// Pulse rate tests
TEST(MonitorTest, PulseHighNotOk) {
  EXPECT_FALSE(vitalsOk(98.6f, 110.0f, 95.0f));
}

TEST(MonitorTest, PulseLowNotOk) {
  EXPECT_FALSE(vitalsOk(98.6f, 55.0f, 95.0f));
}

TEST(MonitorTest, PulseNormalOk) {
  EXPECT_TRUE(vitalsOk(98.6f, 75.0f, 95.0f));
}

// SpO2 tests
TEST(MonitorTest, Spo2LowNotOk) {
  EXPECT_FALSE(vitalsOk(98.6f, 75.0f, 85.0f));
}

TEST(MonitorTest, Spo2NormalOk) {
  EXPECT_TRUE(vitalsOk(98.6f, 75.0f, 95.0f));
}

// Combined test
TEST(MonitorTest, AllVitalsNormalOk) {
  EXPECT_TRUE(vitalsOk(98.1f, 70.0f, 98.0f));
}
